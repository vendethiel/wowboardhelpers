...
var node, replaceWith, template, fetchSiblings, QSA, QS, forumOptions, topic, ref$, forum, tbodyRegular, l, langs, lang, i$, len$, k, timeTable, content, allRead, buttonMar, buttonSticky, x$, firstTopicId, trHtml, aEndHtml, tbodyHtml, y$, h1, refresh, timeout, checkUpdates, characters, res$, name, ref1$, lastPostTh, TSTATE_UNK, TSTATE_ALR, TSTATE_CHK, hasUnread, post, children, div, a, td, lastPostTd, topicId, pages, lastPost, lastPostLink, replies, author, postCount, postOnly, text, isCm, that, authorName, inlineText, simplifiedTime, state, status, tr, hiddenTopics, postPages, units, timestamp, postTitles, postTitle, total, j$, len1$, timespan, count, unit, lastPosterName, infos, realm, extensions, rules, replace, h, e, textarea, submit, clearer, z$, keyCode, memes, postWrapper, addMeme, appendMeme, memebox, ul, out$ = typeof exports != 'undefined' && exports || this, split$ = ''.split, replace$ = ''.replace, join$ = [].join, slice$ = [].slice;
...
y$ = QS('#forum-actions-top');
y$.insertBefore(h1 = node('h1'), (ref$ = y$.children)[ref$.length - 1]);
refresh = function(){
  var x$;
...
for (i$ = 0, len$ = (ref$ = QSA('tbody.regular .post-pages')).length; i$ < len$; ++i$) {
  ...
  (fn1$.call(this, tr, topicId, postPages));
}
...
textarea = z$.querySelector('textarea');
z$.insertBefore(clearer, textarea);
...
function fn1$(tr, topicId, postPages){
  var x$;
